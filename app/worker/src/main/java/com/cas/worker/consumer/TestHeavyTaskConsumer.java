...
import com.cas.common.infra.gameobject.GameObject;
import com.cas.common.infra.gameobject.GameObjectParser;
...
/**
 * TestHeavyTask Consumer (일반 Task - 순차성 보장 X)
 *
 * Kafka에서 TestHeavyTask를 수신하여 Redis의 TestObject를 조회하고 로그로 출력합니다.
 * BaseTaskConsumer를 상속받아 공통 로직을 재사용합니다.
 * 
 * HeavyTask를 직접 상속받은 작업이므로:
 * - 순차성 보장이 없음
 * - 빠른 병렬 처리
 * - Redis 큐잉 오버헤드 없음
 */
@Slf4j
@TaskConsumer(description = "TestHeavyTask를 처리하는 Consumer (일반 Task, 순차성 보장 X)")
@RequiredArgsConstructor
public class TestHeavyTaskConsumer extends BaseTaskConsumer<TestHeavyTask> {
    ...
    public void consume(String message) {
        handleMessage(message, TestHeavyTask.class);
    }
     ...
     */
    @Override
    protected void processTask(TestHeavyTask task) throws Exception {
        log.info("========================================");
        log.info("  Processing TestHeavyTask: {} (General Task)", task.getTaskId());
        log.info("  Sequencing: NO (Fast Parallel Processing)");
        log.info("========================================");
        ...
        if (gameObjectJson == null) {
            ...
            task.setStatus(TestHeavyTask.TaskStatus.FAILED);
    ...
    private void logTestObjectDetails(TestObject testObject) {
        ...
        log.info("  Task Type: TestHeavyTask (General, No Sequencing)");
        log.info("========================================");
